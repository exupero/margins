; A modified version of https://raw.githubusercontent.com/clojure/tools.reader/master/src/main/cljs/cljs/tools/reader.cljs
...
(defn- read-dispatch [^not-native rdr _ opts pending-forms]
...
(defn read-regex [^not-native rdr ch opts pending-forms]
  (let [sb (StringBuffer.)]
...
(defn- read-string* [^not-native reader _ opts pending-forms]
...
(defn- read-keyword [^not-native reader initch opts pending-forms]
...
(defn- read-backtick-string [rdr _ opts pending-forms]
  (loop [items []
         ...
         ch (read-char rdr)]
    (case ch
      nil (err/throw-eof-reading rdr :backtick-string \` (str items))
      \\ (recur items
             ...
             \{ (let [form (read* rdr true nil opts pending-forms)]
                  (if (= (read-char rdr) \})
                    (recur (conj items (str sb) form)
                           (StringBuffer.)
                           (read-char rdr))
                    (err/throw-eof-reading rdr :backtick-string \` (str items))))
             \( (let [form (read* (doto rdr (unread ch)) true nil opts pending-forms)]
                  (recur (conj items (str sb) form)
                         (StringBuffer.)
                         (read-char rdr)))))
      \` (if (zero? (.getLength sb))
           items
           (conj items (str sb)))
...
(defn- read-cond [^not-native rdr _ opts pending-forms]
...
(defn- read-unquote [^not-native rdr comma opts pending-forms]
...
(defn- read-syntax-quote [rdr backquote opts pending-forms]
  (binding [gensym-env {}]
...
(defn- read*-internal [^not-native reader ^boolean eof-error? sentinel return-on opts pending-forms]
  (loop []
    (log-source reader
